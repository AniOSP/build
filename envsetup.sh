...
EOF
    ...
    local A=""
    local i
    for i in `cat $T/build/envsetup.sh $T/vendor/aniosp/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      A="$A $i"
    done
    echo $A
}
...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/vendor/aniosp/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/vendor/aniosp/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^aniosp_") ; then
        ANIOSP_BUILD=$(echo -n $1 | sed -e 's/^aniosp_//g')
    else
        ANIOSP_BUILD=
    fi
    export ANIOSP_BUILD
...
function lunch()
{
    ...
    then
        # if we can't find a product, try to grab it off the AniOSP GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/aniosp/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/aniosp/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
if [ -z ${CCACHE_EXEC} ]; then
    ...
    if [ ! -z "$ccache_path" ]; then
        ...
        echo -e "\e[31m\e[1mccache not found/installed! Not enabling.\e[0m"
    fi
fi

export ANDROID_BUILD_TOP=$(gettop)

. $ANDROID_BUILD_TOP/vendor/aniosp/build/envsetup.sh
